...
int main(int argc, char *argv[])
{
	...
	// parse arguments
	opterr = 0;
	while ((c = getopt(argc, argv, "argmtwlh")) != -1){
		switch (c){
		case 'a':
			g_mode = G_MODE_RAW;
			a_mode = A_MODE_RAW;
	...
	while (running) {
		...
		if(log_raw_values){
			...
			fprintf(logfile, "%06d,%06d,%06d,",\
							...
							data.raw_gyro[1],\
							data.raw_gyro[2]);
			fprintf(logfile, "%06.4f,%06.4f,%06.4f,%06.4f\n",\
							data.dmp_quat[0],\
							data.dmp_quat[1],\
							data.dmp_quat[2],\
							data.dmp_quat[3]);
